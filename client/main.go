...
func main() {
	...
	// (Windows), a single-instance guard, and no inactivity auto-shutdown. An
	// add-on-spawned Client (which always passes --version) keeps the original
	// behaviour.
	...
	// Enforce a single running instance per version for standalone Clients (or
	// when explicitly requested). Must happen before binding the port so a
	// duplicate exits cleanly instead of failing with "address in use".
	if *singleInstance || standalone {
...
func DownloadPrxc(t *Task, wg *sync.WaitGroup) {
	...
	_, signedURL, _, err := GetSignedURL(data.SceneUUID, file, data.APIKey, data.AddonVersion, data.PlatformVersion, t.AppID)
	if err != nil {
		t.Status = "error"
	...
	headers := getHeaders(data.APIKey, *SystemID, data.AddonVersion, data.PlatformVersion, t.AppID)
	req.Header = headers
...
func CheckHealth(data MinimalTaskData) {
	...
	AddTaskCh <- task
	headers := getHeaders(data.APIKey, *SystemID, data.AddonVersion, data.PlatformVersion, data.AppID)
...
func GetDownloadURLWrapper(w http.ResponseWriter, r *http.Request) {
	...
	canDownload, URL, _, err := GetDownloadURL(data.Preferences.SceneID, data.DownloadAssetData.Files, data.Preferences.Resolution, data.Preferences.APIKey, data.AddonVersion, data.PlatformVersion, data.AppID)
	if err != nil {
...
func DownloadGravatarImage(data FetchGravatarData) {
	...
	headers := getHeaders("", *SystemID, data.AddonVersion, data.PlatformVersion, data.AppID)
	req.Header = headers
...
func UploadAssetData(files []UploadFile, data AssetUploadRequestData, metadataResp AssetsCreateResponse, isMainFileUpload bool, taskID string) (json.RawMessage, error) {
	...
	req.Header = getHeaders(data.Preferences.APIKey, *SystemID, data.UploadData.AddonVersion, data.UploadData.PlatformVersion, data.AppID)
...
func uploadFileToS3(file UploadFile, uploadInfo S3UploadInfoResponse, appID int, taskID, apiKey, addonVersion, platformVersion string) error {
	...
	valReq.Header = getHeaders(apiKey, *SystemID, addonVersion, platformVersion, appID)
...
func PackBlendFile(data AssetUploadRequestData, metadata AssetsCreateResponse, isMainFileUpload bool) ([]UploadFile, error) {
	...
	for _, filetype := range upload_set {
		...
		if filetype == "photo_thumbnail" {
			...
			if export_data.PhotoThumbnailPath != "" {
				file := UploadFile{
					Type:     "photo_thumbnail",
					Index:    0,
					FilePath: export_data.PhotoThumbnailPath,
				}
				files = append(files, file)
			} else {
...
func CreateMetadata(data AssetUploadRequestData) (*AssetsCreateResponse, json.RawMessage, error) {
	...
	headers := getHeaders(data.Preferences.APIKey, *SystemID, data.UploadData.AddonVersion, data.UploadData.PlatformVersion, data.AppID)
...
func bkclientjsGetAsset(appID int, apiKey, assetBaseID, assetID, resolution string, targetSoftware Software) {
	...
	res := resolution
	if targetSoftware.Resolution != "" {
		res = targetSoftware.Resolution
	}
	selectedFile, _ := selectAssetFile(assetData.Files, assetData.AssetType, targetSoftware.ModelFormat, res)
	canDownload, downloadURL, _, err := GetSignedURL(sceneID, selectedFile, apiKey, targetSoftware.AddonVersion, "", appID)
	if err != nil {
	...
	req.Header = getHeaders("", *SystemID, targetSoftware.AddonVersion, "", appID)
