...
class MtlxTraversal:
    ...
    def findEdge(edge, processedEdges):
        ...
        for pe in processedEdges:
            ...
            if (
                pe.getUpstreamElement() == edge.getUpstreamElement()
                and pe.getDownstreamElement() == edge.getDownstreamElement()
                and pe.getConnectingElement() == edge.getConnectingElement()
            ):
                return True
        return False
...
class MtlxGraphBuilder:
        ...
        if interfaceName:
            if len(parentGraphPath) == 0:
                ...
                if not found:
                    ...
                    self.updateGraphDictionaryPath(
                        parentGraphPath,
                        parentGraphPath + "/" + interfaceName,
                        "input",
                        port.getType(),
                        itemValue,
                        self.graphDictionary,
                ...
                result = [
                    self.appendPath(parentGraphPath, interfaceName),
                    outputName,
                    destNode,
                    destPort,
                    "interfacename",
    ...
    def buildConnections(self, doc, graphElement, connections):
        ...
        for elem in graphElement.getChildren():
            if not elem.hasSourceUri():
                if elem.isA(mx.Input) or elem.isA(mx.Output):
    ...
    def buildGraphDictionary(self, doc):
        ...
        for elem in doc.getChildren():
            if elem.hasSourceUri():
            ...
            elif elem.isA(mx.Input) or elem.isA(mx.Output) or elem.isA(mx.Node):
                ...
                for node in elem.getTokens():
                    self.updateGraphDictionaryItem(node, graphDictionary)
            elif elem.isA(mx.NodeDef) or elem.isA(mx.Token):
                self.updateGraphDictionaryItem(elem, graphDictionary)
    ...
    def exportToJSON(self, filename, inputFileName):
        ...
        data["copyright"] = "Copyright 2024, NanMu Consulting. kwokcb@gmail.com"
    ...
    def emitInterfaceInputs(indent, edge, subgraphs, edgeOutput, styleOutput):
        ...
        for input in upstreamElem.getInputs():
            ...
            if interfaceInput:
                ...
                outVal = (
                    indent + interfaceNameM + "([" + interfaceName + ']) ==".' + input.getName() + '"==> ' + nodeName
                )
                if outVal not in edgeOutput:
                    edgeOutput.add(outVal)
        ...
        if connectingElem:
            ...
            if outputString:
                ...
                if upstreamOutput:
                    ...
                    outVal = (
                        indent + upstreamPathM + leftBrace + upstreamPath + rightBrace + " --> " + outConnectionString
                    )
                    if outVal not in edgeOutput:
                        edgeOutput.add(outVal)
